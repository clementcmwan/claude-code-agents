...
Behavioral guidelines to reduce common LLM coding mistakes. These apply to all agents.
...
### Primary Agents
- **Main-Agent**: Coordinates all tasks, delegates to subagents
...
Main-Agent
    |
Master-Planner (understand requirements)
    |
Main-Agent (implement solution)
    |
Logger (add logging - if needed)
    |
Reviewer (static analysis)
    |
Main-Agent (fix issues)
    |
Tester (runtime validation)
    |
Main-Agent (fix errors)
    |
Result (validated, instrumented code)
```
